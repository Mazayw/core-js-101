...
function timeSpanToString(startDate, endDate) {
  ...
  const msec = diff.getUTCMilliseconds();
  const sHours = hours < 10 ? `0${hours}` : hours;
  const sMin = min < 10 ? `0${min}` : min;
  const sSec = sec < 10 ? `0${sec}` : sec;
  const sMsec = `${msec}${'0'.repeat(3 - msec.toString().length)}`;

  return `${sHours}:${sMin}:${sSec}.${sMsec}`;
}
...
function angleBetweenClockHands(date) {
  ...
  let angle = Math.abs(hours * 30 - min * 6 + min / 2);
